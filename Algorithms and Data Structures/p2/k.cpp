...
void func( int* arr, int size, int sum ){
    ...
    for( int i = 0; i < size; i++ )
    {
        ...
        //idziemy po różnicach
        int* new_tab = new int[sum];
        for( int j = 0; j < max_calculated + 1; j++ ){
            if( towers[j] == -1 ) //nie ma klocka o takiej różnicy wysokości więc nie ma co dokładać
                continue;
            ...
            new_max_calc = max( new_max_calc, new_diff );
        }
