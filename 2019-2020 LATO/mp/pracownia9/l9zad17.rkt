...
;;Pracowałem wspólnie z Jakubem Szajnerem i Kacprem Gawdzińskim
; --------- ;
; Wyrazenia ;
; --------- ;
...
(env-lookup 'z (eval-while (parse-while '{(x := 5)
                                       (f := (lambda (y) (+ x y)))
                                       (x := 10)
                                       (z := (f 0))}) env-empty))
