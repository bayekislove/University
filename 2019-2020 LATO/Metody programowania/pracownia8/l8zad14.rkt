#lang racket
(provide parse eval)
;Zadanie wykonane wspólnie z Kacprem Gawdzińskim i Jakubem Szajnerem
...
(define (parse-apply lambersy argersy)
  (if (lam? (lam-e lambersy))
      (cond [(equal? (eval (cdr-expr  argersy)) '())
             (app lambersy
                  (car-expr argersy))]
            [else
             (app (lam (lam-id lambersy)
                       (parse-apply (lam-e lambersy)
                                    (cdr-expr argersy)))
                  (car-expr argersy))])
      (cond [(equal? (eval (cdr-expr  argersy)) '())
             (app lambersy (car-expr argersy))]
...
(eval (parse '(apply (lambda (x y) (/ x y)) (list 5 1))))                            
