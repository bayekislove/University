from random     import randint

words = set()

for line in open('words_for_ai1.txt', encoding='utf-8'):
    words.add(line.strip('\n'))

def pom( inp ):
    calculated = dict()

    def find_opt_division( inp ):
        if( inp == '' ):
            return ('', 0)
        starters = list()
        helper = ''
        for i in range( len( inp ) ):
            helper += inp[i]
            if helper in words:
                if( inp[ (i+1): ]) in calculated: #we now how divide remaining string in the best way
                    pom_string = calculated.get( inp[ (i+1): ])[0] 
                    pom_result = calculated.get( inp[ (i+1): ])[1]
                else:
                    pom = find_opt_division( inp[ (i+1): ] )
                    if( pom == None ):
                        continue
                    pom_string = pom[0]
                    pom_result = pom[1]
                starters.append( (helper, pom_string, len(helper)**2 +  pom_result) )
        if( starters == [] ):
            return None
        best_pair = ('', '', 0)
        for guess in starters:
            if( guess[2] > best_pair[2] ):
                best_pair = guess
        calc_string = best_pair[0] + ( ' ' if best_pair[1] != '' else '' ) + best_pair[1]
        calculated[inp] = ( calc_string, best_pair[2] )
        return ( calc_string, best_pair[2] )

    return find_opt_division(inp)[0]

def func( inp ):
    rem = dict()

    def losowy( inp ):
        starters = []
        helper = ''
        if inp == '':
            return -1

        for i in range( len(inp) ):
            helper += inp[i]
            if helper in words:
                if( inp[ (i+1): ] in rem ):
                    res = rem.get( inp[ (i+1): ] )
                else:
                    res = losowy( inp[ (i+1): ] )
                    if res != []:
                        if res == -1: 
                            starters.append( helper )
                        else:
                            starters.append( helper + " " + res )
        if starters != []:
            rem[inp] = starters[ randint(0, len(starters)-1 ) ]
            return rem[inp]
        return []

    return losowy( inp )

lines = []
for asda in open( 'real_slim_tadeusz.txt', encoding='utf-8-sig' ):
        lines.append( asda.strip('\n') )
del lines[0]
lines.insert( 0, 'księga pierwsza' )

def tadeusz():
    match = 0
    total = 0

    for line in open( 'bezspacji.txt', encoding='utf-8' ):
        if( lines[total] == pom( line.strip('\n') ) ):
            match += 1
        total += 1
    return match / len( lines )

def losowe():
    match = 0 
    total = 0

    for line in open( 'bezspacji.txt', encoding='utf-8' ):
        if( lines[total] == func( line.strip('\n') ) ):
            match += 1
        total += 1
    return match / len( lines )

print( func("tamatematyka") )
print( "Pasujące wg podziałów z p1: " + str( tadeusz() ) )
print( "Pasujące wg losowego podziału: " + str( losowe() ) )