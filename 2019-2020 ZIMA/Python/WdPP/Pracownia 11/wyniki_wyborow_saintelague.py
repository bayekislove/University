...
for okreg in range(1, 42):
    ...
    for i in range(1,7): #dla kazdego komitetu wyznaczamy ilorazy
        for k in range(int(tabela[okreg][0])): ##tutaj wyznaczamy tyle ilorazow, ile mandatow jest do zdobycia
            if tabela[okreg][i] == '–':
                continue
            ilor_kom = []
            iloraz_pom = float(tabela[okreg][i]) / (2*k + 1) #wyznaczamy odpowiedni k-ty iloraz
